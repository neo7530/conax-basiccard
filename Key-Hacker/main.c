...
uint8_t systemkey[16] = {0x10,0x11,0x12,0x13,0x14,0x15,0x16,0x17,0x18,0x19,0x20,0x21,0x22,0x23,0x24,0x25};
...
do{
    ...
    printf("\n");
}while(l != 0x9000 && l != 0x9011 && l != 0x9017);
}
...
uint8_t emmbuffer[0x50] ={
  ...
  if(RS232_OpenComport(cport_nr, bdrate, mode, 0))
  {
    ...
    command("\xdd\x26\x00\x00\x03","\x69\x01\x00",100);
    command("\xdd\x26\x00\x00\x03","\x10\x01\x40",100);
    command("\xdd\x26\x00\x00\x03","\x1c\x01\x00",100);
    command("\xdd\x82\x00\x00\x14","\x11\x12\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\x09\x04\x0B\x00\xFF\xFF\xFF\xFF\xFF\xFF",100);
    ...
    unsigned char cmd[5] = "\xdd\xF1\x00\x00\x03";
    cmd[4] = strlen(argv[index])+16;


    command(cmd,updatebuffer,1000);
